        ...
        <AlertDialogPrimitive.Content
            ...
            className={cn(
                `fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%]
                 translate-y-[-50%] gap-4 border border-slate-200 bg-white py-4 shadow-lg 
                 ...
                 data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95
                  ...
                  data-[state=open]:slide-in-from-top-[48%] sm:rounded
...
const AlertDialogFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
    <div
        className={cn('flex flex-col-reverse sm:flex-row justify-end sm:space-x-2 px-4 pt-4 border-t', className)}
        {...props}
    />
);
...
>(({ className, ...props }, ref) => (
    <AlertDialogPrimitive.Description ref={ref} className={cn('text-base px-4 py-2', className)} {...props} />
));
...
const AlertDialogCancel = React.forwardRef<
    ...
    React.ComponentPropsWithoutRef<typeof AlertDialogPrimitive.Cancel> & {
        variant?: 'default' | 'destructive' | 'outline' | 'secondary' | 'ghost' | 'link' | 'light' | 'removal';
    }
